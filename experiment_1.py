...
def get_set(log, discover_function):
    ...
    set_result = get_worst_performing_set(tbr_diagnostics)
    return set_result['case_id'].tolist()
...
def get_log_skeleton_set(log, noise_threshold=0.1):
    ...
    set_result = get_worst_performing_set(conformance_lsk, 'dev_fitness')
    return set_result['case_id'].tolist()
...
def get_declare_set(log, min_confidence_ratio = 0, min_support_ratio = 0):
    ...
    set_result = get_worst_performing_set(conf_result, 'dev_fitness')
    return set_result['case_id'].tolist()
...
def run(log_path):
    ...
    title = log_path.split('/')[-1].split('.')[0]
    pyplot.title(title)
